...
try:
    # file with only the best solution found so far at each evaluation, with columns x1,x2,...,xn,f
    df = pd.read_csv(f'{PREFIX}report_best.csv', header='infer')
    x_values = df.iloc[:, 0].values
    y_values = df.iloc[:, 1].values
    f_values = df.iloc[:, -1].values  # last column is the function value
    i_values = np.arange(len(f_values))
    has_best = True
except:
    has_best = False

# file with ALL the solutions evaluated, with columns x1,x2,...,xn,f
...
f_values_all = df_all.iloc[:, -1].values  # last column is the function value
